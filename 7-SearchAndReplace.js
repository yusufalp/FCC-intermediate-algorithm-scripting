 ...
 * when you are replacing it. For example if you mean to replace the word Book 
 * with the word dog, it should be replaced as Dog
 */

function myReplace(str, before, after) {
  let beforeFirLet = before.charAt(0);
  let afterFirLet = after.charAt(0);

  if (beforeFirLet === beforeFirLet.toLowerCase() &&
    afterFirLet === afterFirLet.toUpperCase()) {
    after = `${afterFirLet.toLowerCase()}${after.substring(1)}`;
  }

  if (beforeFirLet === beforeFirLet.toUpperCase() &&
    afterFirLet === afterFirLet.toLowerCase()) {
    after = `${afterFirLet.toUpperCase()}${after.substring(1)}`;
  }
